...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/16 18:42:22 by wooyang           #+#    #+#             */
/*   Updated: 2025/11/16 18:42:24 by wooyang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_init_philo(t_table *table)
{
	...
	while (i < table->nbr_philo)
	{
		...
		if (i % 2 == 0)
		{
			table->philo[i].fork_le = &table->fork[i];
			table->philo[i].fork_2e
				= &table->fork[(i + 1) % table->nbr_philo];
		}
		else if (i % 2 == 1)
		{
			table->philo[i].fork_le = &table->fork[(i + 1) % table->nbr_philo];
			table->philo[i].fork_2e = &table->fork[i];
		}
		i++;
		j++;
	}
