...
typedef struct s_counter
{
 ...
 pthread_mutex_t count_mutex_autre;
 unsigned int count;
} t_counter;
...
// Cette fois ci, on protege les variable grace a un mutex qu'on appelle count_mutex,
// c'est une variable de type pthread_mutex_t et doit etre initialiser avec pthread_mutex_init()
// maintenant avant d'utilise une variable partagee entre plusieurs thread, 
// on utilise la fonction pthread_mutex_lock(addresse du mutex) pour mettre les autres thread en attente tant que c'est lock
// le thread qui a la ligne pour mutex lock en premier va continuer a s'executer normalement
// et les autres thread qui arriveront a la ligne mutex lock seront en attente car un autre thread a deja lock et attendront tant que ca n'a pas ette unlock
// puis on utilise pthread_mutex_unlock(addresse du mutex) pour debloquer les autres threads pour qu'ils puissent a leur acceder a la variable
